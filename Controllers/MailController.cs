...
public class MailController : Controller
{
    ...
    public MailController( IMailRepository mailRepository,IMailStatusRepository mailStatusRepository,IHistoryRepository    historyRepository,IUserRepository       userRepository,AgencyService         agencyService)
    {
    ...
    public async Task<IActionResult> Index()
    {
        ...
        var latestStatusDictionary = BuildLatestStatusDictionary(await _mailStatusRepository.GetMailStatusesByMailIdsAsync(mailIds));
        ...
        var viewModel = new MailIndexViewModel
        {
            ...
            InboxMails  = inboxMails .Select(m => ToSummary(m, latestStatusDictionary)),
            OutboxMails = outboxMails.Select(m => ToSummary(m, latestStatusDictionary))
        };
    ...
    public async Task<IActionResult> Details(int? id)
    {
        ...
        var latestStatus = await _mailStatusRepository.GetLatestMailStatusAsync(mail.Id);

        ViewBag.CurrentStatus = MapStatusLabel(latestStatus?.status);
        ViewBag.Statuses      = await _mailStatusRepository.GetMailStatusesAsync(mail.Id);
    ...
    public async Task<IActionResult> Create(Mail mail)
    {
        ...
        {
            mail.DateReceived = mail.DateSent.AddDays(mail.MailTypeId switch
            {
                4 => 1, 3 => 2, 2 => 3, _ => 5
            });
        }
        ...
        await _mailStatusRepository.AddMailStatusAsync(new MailStatus
        {
            status = "En attente",
            Type   = "Automatique",
            MailId = mail.Id
        ...
        await _mailStatusRepository.SaveAsync();
    ...
    public async Task<IActionResult> DeleteConfirmed(int id)
    {
        ...
        await _mailStatusRepository.DeleteMailStatusesByMailIdAsync(id);
    ...
    public async Task<IActionResult> Dashboard()
    {
        ...
        var mailIds = mails.Select(m => m.Id);
        var latestStatusDictionary = BuildLatestStatusDictionary(
            await _mailStatusRepository.GetMailStatusesByMailIdsAsync(mailIds));
        ...
        var viewModel = new DashboardViewModel
        {
            ...
            SentCount        = mails.Count(m => MapStatusLabel(latestStatusDictionary.GetValueOrDefault(m.Id)) == "Envoyé"),
            DeliveredCount   = mails.Count(m => MapStatusLabel(latestStatusDictionary.GetValueOrDefault(m.Id)) == "Livré"),
            RecentMails      = mails.Take(5).Select(m => ToSummary(m, latestStatusDictionary))
        };
    ...
    private static Dictionary<int, string> BuildLatestStatusDictionary(IEnumerable<MailStatus> statuses)
    {
        return statuses
    ...
    private static MailSummaryViewModel ToSummary(Mail m, Dictionary<int, string> statusDict) => new(){
            Id          = m.Id,
            Reference   = m.Reference,
            Sender      = m.Sender,
            Recipient   = m.Recipient,
            DateSent    = m.DateSent,
            StatusLabel = MapStatusLabel(statusDict.GetValueOrDefault(m.Id))
        };
